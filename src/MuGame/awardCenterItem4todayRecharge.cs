...
namespace MuGame
{
	internal class awardCenterItem4todayRecharge : BaseAwardCenter
	{
		...
		public awardCenterItem4todayRecharge(Transform trans, WelfareModel.itemWelfareData iwd) : base(trans)
		{
			this.root = trans;
			this.m_iwd = iwd;
		...
		{
			BaseProxy<welfareProxy>.getInstance().sendWelfare(welfareProxy.ActiveType.dayRechargeAward, this.m_iwd.id);
		}
	}
}
