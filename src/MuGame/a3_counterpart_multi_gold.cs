...
namespace MuGame
{
	internal class a3_counterpart_multi_gold : a3BaseActive
	{
		...
		{
			Variant variant = SvrLevelConfig.instacne.get_level_data(109u);
			int num = variant["daily_cnt"];
			int num2 = 0;
			bool flag = MapModel.getInstance().dFbDta.ContainsKey(109);
			if (flag)
			{
				num2 = Mathf.Min(MapModel.getInstance().dFbDta[109].cycleCount, num);
			}
