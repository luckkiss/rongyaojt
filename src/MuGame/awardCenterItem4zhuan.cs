...
namespace MuGame
{
	internal class awardCenterItem4zhuan : BaseAwardCenter
	{
		...
		public awardCenterItem4zhuan(Transform trans, WelfareModel.itemWelfareData iwd) : base(trans)
		{
			...
			this.txtInfo.text = string.Format(iwd.desc, iwd.zhuan);
		...
		{
			BaseProxy<welfareProxy>.getInstance().sendWelfare(welfareProxy.ActiveType.lvlAward, this.m_iwd.id);
		}
	}
}
