...
namespace MuGame
{
	internal class a3_counterpart_multi_exp : a3BaseActive
	{
		...
		{
			Variant variant = SvrLevelConfig.instacne.get_level_data(108u);
			int num = variant["daily_cnt"];
			int num2 = 0;
			bool flag = MapModel.getInstance().dFbDta.ContainsKey(108);
			if (flag)
			{
				num2 = Mathf.Min(MapModel.getInstance().dFbDta[108].cycleCount, num);
			}
