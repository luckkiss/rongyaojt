...
public static class ToLuaExport
{
	...
	{
		ToLuaExport.<Generate>c__AnonStorey1A <Generate>c__AnonStorey1A = new ToLuaExport.<Generate>c__AnonStorey1A();
		...
		if (ToLuaExport.type.IsEnum)
		{
			...
			ToLuaExport.SaveFile(AppConst.LuaWrapPath + ToLuaExport.wrapClassName + "Wrap.cs");
			return;
		}
		...
		<Generate>c__AnonStorey1A.ps = ToLuaExport.type.GetProperties();
		int i;
		for (i = 0; i < <Generate>c__AnonStorey1A.ps.Length; i++)
		{
			int num = list.FindIndex((MethodInfo m) => m.Name == "get_" + <Generate>c__AnonStorey1A.ps[i].Name);
			if (num >= 0 && list[num].Name != "get_Item")
			{
				list.RemoveAt(num);
			}
			num = list.FindIndex((MethodInfo m) => m.Name == "set_" + <Generate>c__AnonStorey1A.ps[i].Name);
		...
		ToLuaExport.SaveFile(luaWrapPath + ToLuaExport.wrapClassName + "Wrap.cs");
	}
	...
	{
		ToLuaExport.<GenConstruct>c__AnonStorey1D <GenConstruct>c__AnonStorey1D = new ToLuaExport.<GenConstruct>c__AnonStorey1D();
		...
		<GenConstruct>c__AnonStorey1D.list = new List<ConstructorInfo>();
		for (int k = 0; k < constructors.Length; k++)
		{
			if (!ToLuaExport.HasDecimal(constructors[k].GetParameters()))
			{
				if (!ToLuaExport.IsObsolete(constructors[k]))
				{
					...
					int num = <GenConstruct>c__AnonStorey1D.list.FindIndex((ConstructorInfo p) => ToLuaExport.CompareMethod(p, r) >= 0);
					if (num >= 0)
					{
						if (ToLuaExport.CompareMethod(<GenConstruct>c__AnonStorey1D.list[num], r) == 2)
						{
							<GenConstruct>c__AnonStorey1D.list.RemoveAt(num);
							<GenConstruct>c__AnonStorey1D.list.Add(r);
						}
					}
					else
					{
		...
		if (<GenConstruct>c__AnonStorey1D.list.Count == 0)
		{
			if (!ToLuaExport.type.IsValueType)
			{
				ToLuaExport.NoConsturct();
		...
		<GenConstruct>c__AnonStorey1D.list.Sort(new Comparison<ConstructorInfo>(ToLuaExport.Compare));
		ToLuaExport.sb.AppendLine("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
		ToLuaExport.sb.AppendFormat("\tstatic int _Create{0}(IntPtr L)\r\n", ToLuaExport.wrapClassName);
		ToLuaExport.sb.AppendLine("\t{");
		ToLuaExport.sb.AppendLine("\t\tint count = LuaDLL.lua_gettop(L);");
		...
		int i;
		for (i = 0; i < <GenConstruct>c__AnonStorey1D.list.Count; i++)
		{
			int num2 = <GenConstruct>c__AnonStorey1D.list.FindIndex((ConstructorInfo p) => p != <GenConstruct>c__AnonStorey1D.list[i] && p.GetParameters().Length == <GenConstruct>c__AnonStorey1D.list[i].GetParameters().Length);
			if (num2 >= 0 || (ToLuaExport.HasOptionalParam(<GenConstruct>c__AnonStorey1D.list[i].GetParameters()) && <GenConstruct>c__AnonStorey1D.list[i].GetParameters().Length > 1))
			{
				list.Add(<GenConstruct>c__AnonStorey1D.list[i]);
			}
		}
		MethodBase methodBase = <GenConstruct>c__AnonStorey1D.list[0];
		bool flag = <GenConstruct>c__AnonStorey1D.list[0].GetParameters().Length == 0;
		...
		else
		{
			...
			if (<GenConstruct>c__AnonStorey1D.list.Count == 1 || methodBase.GetParameters().Length != <GenConstruct>c__AnonStorey1D.list[1].GetParameters().Length)
			{
		...
		ToLuaExport.sb.AppendLine("\t\t{");
		int num3 = ToLuaExport.ProcessParams(methodBase, 3, true, <GenConstruct>c__AnonStorey1D.list.Count > 1, false);
		ToLuaExport.sb.AppendFormat("\t\t\treturn {0};\r\n", num3);
		ToLuaExport.sb.AppendLine("\t\t}");
		for (int j = 1; j < <GenConstruct>c__AnonStorey1D.list.Count; j++)
		{
			flag = (<GenConstruct>c__AnonStorey1D.list[j].GetParameters().Length == 0 || flag);
			methodBase = <GenConstruct>c__AnonStorey1D.list[j];
			ParameterInfo[] parameters3 = methodBase.GetParameters();
			if (!ToLuaExport.HasOptionalParam(methodBase.GetParameters()))
			{
				if (list.Contains(<GenConstruct>c__AnonStorey1D.list[j]))
				{
	...
	{
		ToLuaExport.<GenOverrideFunc>c__AnonStorey20 <GenOverrideFunc>c__AnonStorey = new ToLuaExport.<GenOverrideFunc>c__AnonStorey20();
		<GenOverrideFunc>c__AnonStorey.list = new List<MethodInfo>();
		for (int k = 0; k < ToLuaExport.methods.Length; k++)
		{
			...
			{
				ToLuaExport.Push(<GenOverrideFunc>c__AnonStorey.list, ToLuaExport.methods[k]);
			}
		}
		if (<GenOverrideFunc>c__AnonStorey.list.Count == 1)
		{
			return <GenOverrideFunc>c__AnonStorey.list[0];
		}
		<GenOverrideFunc>c__AnonStorey.list.Sort(new Comparison<MethodInfo>(ToLuaExport.Compare));
		ToLuaExport.sb.AppendLine("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
		...
		List<MethodInfo> list = new List<MethodInfo>();
		int i;
		for (i = 0; i < <GenOverrideFunc>c__AnonStorey.list.Count; i++)
		{
			int num = <GenOverrideFunc>c__AnonStorey.list.FindIndex((MethodInfo p) => ToLuaExport.CompareParmsCount(p, <GenOverrideFunc>c__AnonStorey.list[i]));
			if (num >= 0 || (ToLuaExport.HasOptionalParam(<GenOverrideFunc>c__AnonStorey.list[i].GetParameters()) && <GenOverrideFunc>c__AnonStorey.list[i].GetParameters().Length > 1))
			{
				list.Add(<GenOverrideFunc>c__AnonStorey.list[i]);
			}
		}
		ToLuaExport.sb.AppendLine();
		MethodInfo methodInfo = <GenOverrideFunc>c__AnonStorey.list[0];
		...
		int num6 = <GenOverrideFunc>c__AnonStorey.list[1].GetParameters().Length + ((!<GenOverrideFunc>c__AnonStorey.list[1].IsStatic) ? 1 : 0);
		bool flag = true;
		...
		ToLuaExport.sb.AppendLine("\t\t{");
		int num7 = ToLuaExport.ProcessParams(methodInfo, 3, false, <GenOverrideFunc>c__AnonStorey.list.Count > 1 && flag, beCheckTypes);
		ToLuaExport.sb.AppendFormat("\t\t\treturn {0};\r\n", num2 + num7);
		ToLuaExport.sb.AppendLine("\t\t}");
		for (int j = 1; j < <GenOverrideFunc>c__AnonStorey.list.Count; j++)
		{
			flag = true;
			beCheckTypes = true;
			methodInfo = <GenOverrideFunc>c__AnonStorey.list[j];
			...
			if (!ToLuaExport.HasOptionalParam(methodInfo.GetParameters()))
			{
				...
				if (list.Contains(<GenOverrideFunc>c__AnonStorey.list[j]))
				{
	...
	public static void GenDelegates(DelegateType[] list)
	{
		...
		ToLuaExport.sb.AppendLine("\t}\r\n");
		ToLuaExport.sb.AppendLine("}");
		ToLuaExport.SaveFile(AppConst.LuaBasePath + "Base/DelegateFactory.cs");
		ToLuaExport.Clear();
	}
	...
	private static void ProcessExtends(List<MethodInfo> list)
	{
		...
		if (ToLuaExport.extendType != null)
		{
			ToLuaExport.<ProcessExtends>c__AnonStorey22 <ProcessExtends>c__AnonStorey = new ToLuaExport.<ProcessExtends>c__AnonStorey22();
			<ProcessExtends>c__AnonStorey.list2 = new List<MethodInfo>();
			<ProcessExtends>c__AnonStorey.list2.AddRange(ToLuaExport.extendType.GetMethods(BindingFlags.Instance | ToLuaExport.binding | BindingFlags.DeclaredOnly));
			int i;
			for (i = <ProcessExtends>c__AnonStorey.list2.Count - 1; i >= 0; i--)
			{
				if ((!<ProcessExtends>c__AnonStorey.list2[i].Name.Contains("op_") && !<ProcessExtends>c__AnonStorey.list2[i].Name.Contains("add_") && !<ProcessExtends>c__AnonStorey.list2[i].Name.Contains("remove_")) || ToLuaExport.IsNeedOp(<ProcessExtends>c__AnonStorey.list2[i].Name))
				{
					list.RemoveAll((MethodInfo md) => md.Name == <ProcessExtends>c__AnonStorey.list2[i].Name);
					if (!ToLuaExport.IsObsolete(<ProcessExtends>c__AnonStorey.list2[i]))
					{
						list.Add(<ProcessExtends>c__AnonStorey.list2[i]);
					}
				}
			}
		}
