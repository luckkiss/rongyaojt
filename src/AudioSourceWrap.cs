...
public class AudioSourceWrap
{
	...
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("isVirtual", new LuaCSFunction(AudioSourceWrap.get_isVirtual), null),
	...
	private static int get_isVirtual(IntPtr L)
	{
		...
		LuaScriptMgr.Push(L, audioSource.isVirtual);
		return 1;
	}
	...
	private static int GetSpatializerFloat(IntPtr L)
	{
		...
		int index = (int)LuaScriptMgr.GetNumber(L, 2);
		float d;
		bool spatializerFloat = audioSource.GetSpatializerFloat(index, out d);
